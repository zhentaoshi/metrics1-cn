...
为什么要讨论条件期望? 一个重要动机是出于预测的目的.

机器学习可谓是包罗万象, 其中包含了许多有用的方法, 回归模型也包含于其中. 且通常被归类为一种**监督式学习** (*supervised leaning*). 在监督式学习中, 我们用$x$的函数$g(x)$来预测$y$, 但除非二者之间的关系是决定式的, 否则$x$并不能完美地预测$y$, 而这一预测误差$y-g(x)$依赖于函数$g$的选择. 但函数$g$的范围实在太广了, 如何找到一个最优的呢? 事实上, 这个问题中我们并不关心相应联合分布$(y,x)$的数据生成过程 (*data generating process*, 即DGP), 我们的目标是找到一种通用的规则, 无论这一对随机变量是如何生成的, 这种规则都能够找到一个合适的函数, 它在给定$x$的情况下能够尽可能精确地预测$y$.

回答这一问题的关键在于, 我们需要选择一种准则以比较不同的$g$, 这种准则被称为**损失函数**, 常记为$L(y,g(x))$. **平方损失** (*quadratic loss*) 是一种非常方便的损失函数, 它的定义为

$$
L(y,g(x))=(y-g(x))^2.
$$
由于数据是随机的, 因此损失函数$L(y,g(x))$也是随机的. 这里的"随机"意味着不确定性, 在不同的时候, 即使在同一分布下也可能产生不同的结果. 为了摆脱这种随机性的影响, 我们在联合分布$(y,x)$的意义下对损失函数取平均,得到$R(y,g(x))=E[L(y,g(x))]$, 我们将其称为**风险**. 与损失函数不同, 风险是一个确定性的量. 对于平方损失函数, 相应的风险
$$
R(y,g(x))=E\left[(y-g(x))^2 \right]
$$

被称为**均方误差** (*mean square error*, 即MSE). 当然, 还有许多可替代的测度, 例如**平均绝对误差** (*mean absolute error*, 即MAE) $E[|y-g(x)|]$, 但最常用的风险测度方法仍然是均方误差MSE.
...
然而, 在现实中, 有且仅有一种情况发生, 正如古希腊哲学家Heraclitus (553 BC--475 BC)所说, "你不能两次踏入同一条河流". 每个个体身上有且仅有一种结果得到实现, 因此个体的处置效应不具有**可操作性** (*operational*), 即无法从总体水平的数据计算出来. 但我们有很多个体的数据, 于是我们可以定义**平均处置效应** (*average treatment effect*, 即ATE)
