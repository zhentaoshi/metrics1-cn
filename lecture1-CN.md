...
``` {admonition} Example
令 $\Omega=\{1,2,3,4,5,6\}$， 令 $\mathcal{A}=\{\{1\},\{1,3\}\}$。
...
积分是数学分析中最基础的数学方法之一。我们已经在本科阶段学习过黎曼积分，黎曼积分是符合数学直觉的。但是勒贝格积分才是更加泛用的积分定义。勒贝格积分的建立依靠以下几个步骤。

1.  令 $a_{i}\in\mathbb{R}$，且令 $\{A_{i}\in\mathcal{F}\}_{i\in\mathbb{N}}$ 。
    如果 $X=\sum_{i}a_{i}\cdot1\left\{ A_{i}\right\}$ 而且它的加总是有限的，则称
    $X$ 为测度空间 $\left(\Omega,\mathcal{F}\right)$ 
    上的**简单函数**。简单函数是可测的。
...
当我们把二元变量中不感兴趣的那一个变量积分掉，使它不再成为函数中的变量，我们就得到**边际密度** $f\left(y\right)=\int f\left(x,y\right)dx$。
...
**支配** (dominating)和 **被支配** (dominated)的关系，我们在此不展开。
...
给定概率空间 $\left(\Omega,\mathcal{F},P\right)$ ，
一个 $\mathcal{F}$-上的满足 $E\left|Y\right|<\infty$ 的可测函数 $Y$，
以及子 $\sigma$ 域 $\mathcal{G}\subseteq\mathcal{F}$。
**条件期望**
 $E\left[Y|\mathcal{G}\right]$ 是满足
所有的 $A\in\mathcal{G}$:
